...
#. Beginning of the script up to the ``ARGBASH_GO`` or ``ARGBASH_PREPARE`` line:
   ...
   - Code that precedes the first Argbash macro is passed through verbatim, square brackets included.
     The exception are lines that contain ``m4sugar`` macros (e.g. the ``m4_ignore`` of templates generated by ``argbash-init``) --- those are expanded as described above.

   - Past the first Argbash macro, one level of square brackets is removed.
     This is the consequence of the previous point --- if you e.g. use a regular expression with square brackets in between Argbash macros,
